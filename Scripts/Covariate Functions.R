...
cov3 <- function(data, time, interest){
  ...
  for(i in 1:length(locations)){
    ...
    if(time == "weeks"){tmp_df <- data.frame(CameraLocation = locations[i],
                                             ...
                                             EndDate = time_seq[2:length(time_seq)]-1, 
                                             n_Max_Ind = 0)}
    if(time == "days"){tmp_df <- data.frame(CameraLocation = locations[i],Date = time_seq , n_Max_Ind = 0)}
    
    # loop to sum maximum number of individuals detected in a single image during 
    # each unique detection event per time step
    for(j in 1:(length(time_seq)-1)){

      tmp_df[j, "n_Max_Ind"]  <- sum(tmp_data[as.Date(tmp_data$DateTime) >= as.Date(time_seq[j]) 
                                          & as.Date(tmp_data$DateTime) < as.Date(time_seq[j+1]),]$n_Max_Ind)
    }
    cov3_list[[i]] <- tmp_df
  }
  return(cov3_list)
